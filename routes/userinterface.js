...
router.post('/user_fetch_ambience_by_city', function(req, res) {
    try {
        const query = `
            ...
            FROM restaurant R
            INNER JOIN restaurantpictures RP ON R.restaurantid = RP.restaurantid
            INNER JOIN timing T ON R.restaurantid = T.restaurantid
            WHERE RP.picturetype = 'Ambience' AND R.cityid = ?`;

        pool.query(query, [req.body.cityid], function(error, result) {
            if (error) {
                console.error("SQL Error Log:", error); // Ye Render logs mein dikhega
                return res.status(500).json({ message: error.sqlMessage, status: false });
            }
            res.status(200).json({ message: 'Success', data: result, status: true });
        });
    } catch (e) {
        res.status(500).json({ message: 'Server Crash Error', status: false });
    }
});
